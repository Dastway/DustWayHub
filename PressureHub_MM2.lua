...
-- ════════════════════════════════════════════════════════════
--  PressureHub · MM2 AutoFarm Module
-- ════════════════════════════════════════════════════════════

guard("mm2_autofarm", function()
	...
	local function getNearestCoin(torso)
		...
		for _, coin in pairs(container:GetChildren()) do
			if coin.Name == "Coin_Server" and coin:IsA("BasePart") and not AFState.ignoredCoins[coin] then
	...
	local function flyToPoint(targetPos, targetCoin, hrp, torso, lockedRotation)
		...
		local tweenInfo = TweenInfo.new(dist / AFSettings.TweenSpeed, Enum.EasingStyle.Linear)
		...
		connection = RunService.Heartbeat:Connect(function()
			if not AFState.isFarming or not targetCoin or not targetCoin:IsDescendantOf(workspace) then
				tween:Cancel()
	...
	local function tweenToCoin(coin)
		...
		if AFState.currentTween then pcall(function() AFState.currentTween:Cancel() end) end

		AFState.currentTween = TweenService:Create(hrp,
			TweenInfo.new(
		...
		local done = false
		local c
		c = AFState.currentTween.Completed:Connect(function()
			done = true
		...
		task.spawn(function()
			while AFState.isFarming do
				...
				local ok, _ = pcall(function()
					...
					if AFSettings.AutoReset then
						local coins = getCurrentCoins()
						if coins >= AFSettings.CoinLimit then
							humanoid.Health = 0
							task.wait(5)
							return
						end
					...
					if AFSettings.FarmMode == "Underground" then
						...
						local targetPos      = targetCoin.Position - Vector3.new(0, AFSettings.UndergroundOffset, 0)
					...
					if reachedTarget and AFState.isFarming and humanoid.Health > 0 then
						...
						task.delay(5, function() AFState.ignoredCoins[targetCoin] = nil end)
						task.wait(0.2)
					end
	...
	-- Noclip keepalive for Underground mode
	NeverLose:AddSignal(RunService.Stepped:Connect(function()
		if not AFState.isFarming or not AFState.isActivelyFlying
	...
	-- ── UI Tab ──────────────────────────────────────────────
	local AFTab = Window:AddTab({ Name = "MM2 FARM", Icon = "coins" })

	local SecFarm   = AFTab:AddSection({ Name = "Auto Farm",  Position = "left"  })
	local SecConfig = AFTab:AddSection({ Name = "Settings",   Position = "right" })

	-- Left section ─ main toggles
	SecFarm:AddLabel("Auto Farm"):AddToggle({
		Default  = false,
		Flag     = "af_enabled",
		Callback = function(v)
			AFSettings.AutoFarmEnabled = v
			if v then startFarming() else stopFarming() end
		end,
	})

	SecFarm:AddLabel("Auto Reset"):AddToggle({
		Default  = AFSettings.AutoReset,
		Flag     = "af_autoreset",
		Callback = function(v)
			AFSettings.AutoReset = v
		end,
	})

	SecFarm:AddLabel("Avoid Murderer"):AddToggle({
		Default  = AFSettings.AvoidMurder,
		Flag     = "af_avoidmurder",
		Callback = function(v)
			AFSettings.AvoidMurder = v
		end,
	})
	...
	SecFarm:AddButton({
		...
		Callback = function()
			stopFarming()
			NeverLose.Flags["af_enabled"]:SetValue(false)
			Notification.new({
				Title   = "MM2 Farm",
	...
	SecConfig:AddLabel("Farm Mode"):AddDropdown({
		Values   = { "Underground", "Sit" },
		Default  = AFSettings.FarmMode,
		Flag     = "af_farmmode",
		Callback = function(v)
			AFSettings.FarmMode = v
		end,
	})

	SecConfig:AddLabel("Tween Speed"):AddSlider({
		Min      = 5,
		Max      = 150,
		...
		Flag     = "af_tweenspeed",
		Callback = function(v)
			AFSettings.TweenSpeed = v
		end,
	})

	SecConfig:AddLabel("Coin Limit (reset)"):AddSlider({
		Min      = 5,
		Max      = 100,
		...
		Flag     = "af_coinlimit",
		Callback = function(v)
			AFSettings.CoinLimit = v
		end,
	})

	SecConfig:AddLabel("Max Coin Distance"):AddSlider({
		Min      = 50,
		Max      = 1000,
		Default  = AFSettings.MaxDistance,
		Type     = "studs",
		Flag     = "af_maxdist",
		Callback = function(v)
			AFSettings.MaxDistance = v
		end,
	})

	SecConfig:AddLabel("Underground Offset"):AddSlider({
		Min      = 0,
		Max      = 15,
		Default  = AFSettings.UndergroundOffset,
		Type     = "studs",
		Flag     = "af_ugoffset",
		Callback = function(v)
			AFSettings.UndergroundOffset = v
		end,
	})

	-- Cleanup on unload
	onUnload("mm2_autofarm", function()
		stopFarming()
	end)

	Notification.new({
		Title   = "PressureHub",
		Content = "MM2 AutoFarm ready — go to the MM2 FARM tab.",
		Duration = 5,
	})
end)
