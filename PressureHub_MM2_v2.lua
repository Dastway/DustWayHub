...
-- ════════════════════════════════════════════════════════════════════
--  PressureHub · MM2 · Clean bootstrap
-- ════════════════════════════════════════════════════════════════════

if getgenv and getgenv().PressureHub then
	pcall(getgenv().PressureHub.Unload)
end
...
assert(type(SOURCE) == "string" and SOURCE ~= "",
	"[PressureHub] SOURCE missing — run the built dist/main.lua")

local libraryChunk = assert(loadstring(SOURCE, "@adapter"))
...
do
	local ok, r = pcall(function() return Font.fromEnum(Enum.Font.GothamMedium) end)
	if ok and r then NeverLose.BuiltInRegular = r end
	local ok2, b = pcall(function() return Font.fromEnum(Enum.Font.GothamBold) end)
	if ok2 and b then NeverLose.BuiltInBold = b end
end

-- ── Черно-красная тема ──────────────────────────────────────────────
-- Применяем ДО создания окна, пока rend-список пуст
do
	local lib = NeverLose.Lib
	if lib and lib.theme then
		lib.theme.accent = Color3.fromRGB(220, 40, 40)   -- красный акцент
		lib.theme.bg     = Color3.fromRGB(8, 8, 10)      -- почти чёрный фон
		lib.theme.side   = Color3.fromRGB(14, 14, 17)    -- левая боковая панель
		lib.theme.panel  = Color3.fromRGB(12, 12, 15)    -- панели
		lib.theme.head   = Color3.fromRGB(18, 18, 22)    -- заголовки
		lib.theme.line   = Color3.fromRGB(60, 18, 18)    -- разделители
		lib.theme.glow   = Color3.fromRGB(220, 40, 40)   -- свечение = красное
		lib.theme.text   = Color3.fromRGB(235, 235, 240) -- белый текст
		lib.theme.dim    = Color3.fromRGB(130, 90, 90)   -- приглушённый
	end
end
...
-- ── TAG / дедупликация ScreenGui ────────────────────────────────────
local TAG = "PressureHubUI"
do
...
local Window = NeverLose:CreateWindow({
	...
	ConfigFolder = "PressureHub/Config",
	Keybind      = "RightShift",
})
...
local hubBlock  = Watermark:AddBlock("zap", "PressureHub")

hubBlock:Input(function() Window:ToggleInterface() end)

-- FPS / Ping счётчик
local _frames, _clock = 0, os.clock()
NeverLose:AddSignal(RunService.RenderStepped:Connect(function()
	_frames = _frames + 1
	local now = os.clock()
	if now - _clock >= 1 then
		fpsBlock:SetText(tostring(_frames) .. " FPS")
		_frames, _clock = 0, now
		local ok, ping = pcall(function()
			return math.floor(LocalPlayer:GetNetworkPing() * 1000)
		end)
		if ok then pingBlock:SetText(tostring(ping) .. " MS") end
	end
end))
...
local function getCurrentCoins()
	local ok, v = pcall(function()
		...
		local mg  = pg:FindFirstChild("MainGUI");        if not mg  then return 0 end
		local gm  = mg:FindFirstChild("Game");           if not gm  then return 0 end
		local cb  = gm:FindFirstChild("CoinBags");       if not cb  then return 0 end
		local con = cb:FindFirstChild("Container");      if not con then return 0 end
		local co  = con:FindFirstChild("Coin");          if not co  then return 0 end
		local cf  = co:FindFirstChild("CurrencyFrame");  if not cf  then return 0 end
		local ic  = cf:FindFirstChild("Icon");           if not ic  then return 0 end
		local ct  = ic:FindFirstChild("Coins");          if not ct  then return 0 end
		return ct.Text
	end)
...
local function getNearestCoin(torso)
	...
	for _, coin in pairs(container:GetChildren()) do
		if coin.Name == "Coin_Server" and coin:IsA("BasePart")
			and not AFState.ignored[coin] then
			local d = (torso.Position - coin.Position).Magnitude
			if d < bestDist and d <= AFSettings.MaxDistance then
...
local function applyFlightPhysics(hrp)
...
local function flyToPoint(targetPos, coin, hrp, torso, rot)
	...
	local info    = TweenInfo.new(dist / AFSettings.TweenSpeed, Enum.EasingStyle.Linear)
	local tween   = TweenService:Create(hrp, info, { CFrame = CFrame.new(targetPos) * rot })
	AFState.tween = tween
	local reached = false
	tween:Play()
	...
	conn = RunService.Heartbeat:Connect(function()
		if not AFState.farming or not coin or not coin:IsDescendantOf(workspace) then
			tween:Cancel(); conn:Disconnect(); return
		end
		...
		if (torso.Position - targetPos).Magnitude <= 1.5 then
			reached = true; tween:Cancel(); conn:Disconnect()
		end
	end)
...
local function tweenSit(coin)
	...
	if AFState.tween then pcall(function() AFState.tween:Cancel() end) end

	local info  = TweenInfo.new(
		(hrp.Position - target).Magnitude / AFSettings.TweenSpeed,
		Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
	local tw    = TweenService:Create(hrp, info, { CFrame = CFrame.new(target) })
	AFState.tween = tw
	hum.Sit = true
	tw:Play()
	...
	while not done and AFState.farming do
		task.wait(0.1)
		if not coin or not coin.Parent then
			pcall(function() tw:Cancel() end); hum.Sit = false; c:Disconnect(); return false
		end
		if tick() - t0 > 30 then
			pcall(function() tw:Cancel() end); hum.Sit = false; c:Disconnect(); return false
		end
...
local function stopFarming()
	AFState.farming    = false
	AFState.flying     = false
	AFState.targetCoin = nil
	if AFState.tween then
		pcall(function() AFState.tween:Cancel() end)
		AFState.tween = nil
	end
	removePhysics()
...
local function startFarming()
	if AFState.farming then return end
	AFState.farming = true
	table.clear(AFState.ignored)

	task.spawn(function()
		while AFState.farming do
			task.wait()
			local ok, _ = pcall(function()

				if nearbyMurderer() then
					AFState.flying = false; AFState.targetCoin = nil
					removePhysics()
				...
				local char = LocalPlayer.Character; if not char then task.wait(1); return end
				local hrp   = char:FindFirstChild("HumanoidRootPart")
				...
				if AFSettings.AutoReset and getCurrentCoins() >= AFSettings.CoinLimit then
				...
				AFState.flying = true; AFState.targetCoin = coin
				local reached  = false

				if AFSettings.FarmMode == "Underground" then
					setupNoclip(char)
					local rot = applyFlightPhysics(hrp)
					local pos = coin.Position - Vector3.new(0, AFSettings.UGOffset, 0)
					reached   = flyToPoint(pos, coin, hrp, torso, rot)

				elseif AFSettings.FarmMode == "Sit" then
					reached = tweenSit(coin)
					if reached and AFState.farming and hum.Health > 0 then
						collectCoin(coin)
					end
				end

				if reached and AFState.farming and hum.Health > 0 then
					AFState.ignored[coin] = true
					task.delay(5, function() AFState.ignored[coin] = nil end)
					task.wait(0.2)
				end
				AFState.targetCoin = nil
...
-- Noclip keepalive (Underground)
NeverLose:AddSignal(RunService.Stepped:Connect(function()
	if not AFState.farming or not AFState.flying
		or AFSettings.FarmMode ~= "Underground" then return end
	local char = LocalPlayer.Character; if not char then return end
	local hum  = char:FindFirstChild("Humanoid")
	if hum then hum.PlatformStand = true end
	for _, p in pairs(char:GetDescendants()) do
...
-- ── Вкладка: AutoFarm ───────────────────────────────────────────────
local TabFarm    = Window:AddTab({ Name = "AUTO FARM", Icon = "coins" })
local SecMain    = TabFarm:AddSection({ Name = "Управление", Position = "left"  })
local SecCfg     = TabFarm:AddSection({ Name = "Настройки",  Position = "right" })

-- Левая секция ─ основные переключатели
SecMain:AddLabel("Автофарм монет"):AddToggle({
	Default  = false,
	Flag     = "af_on",
	Callback = function(v)
		AFSettings.Enabled = v
		if v then startFarming() else stopFarming() end
	end,
})

SecMain:AddLabel("Авто-сброс"):AddToggle({
	Default  = true,
	Flag     = "af_autoreset",
	Callback = function(v) AFSettings.AutoReset = v end,
})

SecMain:AddLabel("Избегать убийцу"):AddToggle({
	Default  = true,
	Flag     = "af_avoidmurder",
	Callback = function(v) AFSettings.AvoidMurder = v end,
})
...
SecMain:AddButton({
	...
	Callback = function()
		stopFarming()
		local t = NeverLose.Flags["af_on"]
		if t and t.SetValue then t:SetValue(false) end
		Notification.new({ Title = "PressureHub", Content = "Фарм остановлен.", Duration = 3 })
	end,
})

-- Правая секция ─ конфиг
local rowMode = SecCfg:AddLabel("Режим фарма")
rowMode:AddDropdown({
	Values   = { "Underground", "Sit" },
	Default  = "Underground",
	Flag     = "af_mode",
	Callback = function(v) AFSettings.FarmMode = v end,
})

local rowSpeed = SecCfg:AddLabel("Скорость движения")
rowSpeed:AddSlider({
	Min      = 5,
	Max      = 150,
	Default  = 25,
	Type     = "ст/с",
	Flag     = "af_speed",
	Callback = function(v) AFSettings.TweenSpeed = v end,
})

local rowLimit = SecCfg:AddLabel("Лимит монет (сброс)")
rowLimit:AddSlider({
	Min      = 5,
	Max      = 100,
	Default  = 40,
	Type     = "монет",
	Flag     = "af_coinlimit",
	Callback = function(v) AFSettings.CoinLimit = v end,
})

local rowDist = SecCfg:AddLabel("Макс. дистанция")
rowDist:AddSlider({
	Min      = 50,
	Max      = 1000,
	Default  = 600,
	Type     = "ст",
	Flag     = "af_maxdist",
	Callback = function(v) AFSettings.MaxDistance = v end,
})

local rowOffset = SecCfg:AddLabel("Смещение Underground")
rowOffset:AddSlider({
	Min      = 0,
	Max      = 15,
	Default  = 4,
	Type     = "ст",
	Flag     = "af_ugoffset",
	Callback = function(v) AFSettings.UGOffset = v end,
})

-- ── Вкладка: Настройки меню ─────────────────────────────────────────
local TabMenu = Window:AddTab({ Name = "НАСТРОЙКИ", Icon = "settings" })
local SecMenu = TabMenu:AddSection({ Name = "Меню", Position = "left" })

SecMenu:AddLabel("Клавиша меню"):AddKeybind({
	Default  = "RightShift",
	Flag     = "menu_key",
	Callback = function(v) Window:SetKeybind(v) end,
})

SecMenu:AddLabel("Ватермарк"):AddToggle({
	Default  = true,
	Flag     = "wm_show",
	Callback = function(v) Watermark:SetRender(v) end,
})
...
SecMenu:AddButton({
	...
	Icon     = "trash-2",
	Callback = function()
		if getgenv then getgenv().PressureHub = nil end
		NeverLose:Unload()
	end,
})
...
NeverLose.Unload = function() Hub.Unload() end
if getgenv then getgenv().PressureHub = Hub end
