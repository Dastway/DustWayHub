...
local function buildHeader(parent, badgeText)
    ...
    task.spawn(function()
        -- 1) пробуем ID как есть
        if waitLoaded(2.5) then
            logoResolved = logoImg.Image
            logoFallback.Visible = false
            return
        end
        if not logoImg.Parent then return end

        -- 2) если это ID Decal — достаём из него настоящую картинку
        ...
        warn("[" .. HUB_NAME .. "] Логотип не загрузился. Проверьте, что LOGO_ID — это ID картинки (Image), она прошла модерацию и доступна.")
    end)
